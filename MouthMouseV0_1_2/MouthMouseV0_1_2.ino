...
int pot1 = 0;    // first analog sensor
int pot2 = 0;   // second analog sensor
int pressureSensor = 0;    // digital sensor
int rightButton = 0;
int leftButton = 0;    // digital sensor
...
void setup()
{
  ...
  pinMode(2, INPUT);   // rightButton is on digital pin 2
  pinMode(3, INPUT); //  leftButton is on digital pin 3
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    pressureSensor = map(analogRead(A3), 0, 1024, 0, 50);
    delay(10);
    // read  switch, map it to 0 or 255L
    leftButton = map(digitalRead(3), 0, 1, 0, 255); 
    // read  switch, map it to 0 or 255L
    rightButton = map(digitalRead(2), 0, 1, 0, 255); 
