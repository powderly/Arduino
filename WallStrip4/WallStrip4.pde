...
#define NUM_LEDS 160                        // Set the Number of LEDs in use here
...
void loop() 
{
  ...
  int whichway = random(1,7);
  
  uint32_t color = 0xFFFFFF;
                       
  if(whichway == 1){ 
    // An example of how pixels and their color values can be s  
    for(Counter=0; Counter < NUM_LEDS ; Counter++)
      {
       BL.setPixel(Counter,color);
       if(Counter > 0){
         BL.setPixel(Counter-1,color);
       }else if(Counter > 2){
         BL.setPixel(Counter-1,color);
         BL.setPixel(Counter-2,color);
         BL.setPixel(Counter,color);
       }
       BL.show();
       delay(10);
       if(Counter == 0){
         BL.setPixel(Counter,0);
       }else if(Counter > 0){
         BL.setPixel(Counter-1,0);
       }else if(Counter > 2){
         BL.setPixel(Counter-1,0);
         BL.setPixel(Counter-2,0);
         BL.setPixel(Counter,0);
       }
  ...
  }else if(whichway>1 && whichway < 4){
    for(Counter=NUM_LEDS; Counter >0 ; Counter--)
      {
       BL.setPixel(Counter,color);
       if(Counter < NUM_LEDS){
         BL.setPixel(Counter+1,color);
       }else if(Counter < NUM_LEDS-2){
         BL.setPixel(Counter+1,color);
         BL.setPixel(Counter+2,color);
         BL.setPixel(Counter,color);
       }
    ...
    for(Counter=0; Counter < NUM_LEDS ; Counter+=flipbit)
      {
       BL.setPixel(Counter,color);
       if(Counter > 0 && Counter <2){
         BL.setPixel(Counter-1,color);
       }else if(Counter > 2 && Counter <= 40){
         BL.setPixel(Counter-1,color);
         BL.setPixel(Counter-2,color);
         BL.setPixel(Counter,color);
       }else if(Counter > 40){
